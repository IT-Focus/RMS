class CashierBalanceService::Service
	def get_cashier_balance user
		...
		if is_admin == true
			@data = CashierBalance.joins({cashier: :sys_user}).where.not(opened_date:nil)
		else
		 	@data = CashierBalance.joins({cashier: :sys_user}).where.not(opened_date:nil).where cashier_id:cashier_id 
		end
